...
const tools = [
  { to: "/color/hsl-to-oklch", label: "HSL → OKLCH" },
  { to: "/color/palette", label: "Palette Generator" },
  { to: "/color/ramp", label: "Ramp" },
  { to: "/color/gradient", label: "Gradient" },
  { to: "/color/hsl-lies", label: "HSL Lies" },
  { to: "/color/contrast", label: "Contrast" },
];

const soon = ["Figma plugin"];
    ...
    <div className="min-h-full grid grid-cols-1 md:grid-cols-[220px_1fr]">
      ...
      <aside className="hidden md:flex flex-col bg-[var(--color-bg)] sticky top-0 h-screen border-r border-[var(--color-border)] overflow-y-auto">
        ...
        <div className="px-6 pb-7">
          <Link to="/color" className="flex items-center gap-2.5">
            <span
            ...
            <span className="font-semibold tracking-tight text-[14px] leading-tight">
              Color is finally <span className="text-[var(--color-fg-dim)]">OK</span>
            </span>
          </Link>
        </div>
      ...
      <header className="md:hidden border-b border-[var(--color-border)] sticky top-0 bg-[var(--color-bg)] z-10">
        <div className="px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <Link to="/color" className="font-semibold tracking-tight text-[13px]">
              Color is finally <span className="text-[var(--color-fg-dim)]">OK</span>
            </Link>
          </div>
        </div>
